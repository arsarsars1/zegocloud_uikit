...
part 'components/cursor_painter.dart';
part 'components/gradiented.dart';
part 'models/dialog_config.dart';
part 'models/pin_theme.dart';

enum DialogConfigPlatform { iOS, other }
...
class AppPinCodeTextField extends StatefulWidget {
  ...
  /// The vertical padding from the [AppPinCodeTextField] to the error text
  /// Default is 16.
  ...
  AppPinCodeTextField({
    ...
    this.keyboardType = const TextInputType.numberWithOptions(decimal: false, signed: false),
...
class _AppPinCodeTextFieldState extends State<AppPinCodeTextField> with TickerProviderStateMixin {
...
// selects the right fill color for the field
  ...
  Future<void> _showPasteDialog(String pastedText) {
    ...
    return showDialog(
      ...
      builder: (context) => _dialogConfig.platform == DialogConfigPlatform.iOS
  ...
  List<Widget> _getActionButtons(String pastedText) {
    ...
    if (_dialogConfig.platform == DialogConfigPlatform.iOS) {
      resultList.addAll([
